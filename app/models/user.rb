class User < ActiveRecord::Base
    ...
    def self.destroy_from_model(hash)
        permissions, role_permissions = get_permissions(:controller => hash[:controller], :action => hash[:action], :token => hash[:token])
        #permissions
            destroy_other_users = role_permissions.include?(permissions[0])
            destroy_itself = role_permissions.include?(permissions[1])
        #end
    	begin
    ...
    def self.update_from_model(hash)
        ...
        #end
    	begin
            user_id = ApiKey.find_by_token(hash[:token]).user.id
    ...
    def self.update_password(hash)
        permissions, role_permissions = get_permissions(:controller => hash[:controller], :action => hash[:action], :token => hash[:token])
        #permissions
            update_other_users_password = role_permissions.include?(permissions[0])
            update_own_user_password = role_permissions.include?(permissions[1])
        #end
        begin
            token_user = ApiKey.find_by_token(hash[:token]).user
            if update_other_users_password || (update_own_user_password && token_user.id == hash[:id].to_i)
                user = self.find(hash[:id])
